...
const ParkingDetail = () => {
  ...
  const [exitQRData, setExitQRData] = useState(null);
  ...
  const handleShowQR = (item) => {
    ...
    // Find matching ENTERED record to get entry time if needed
    const oldRec = data.find(r => (r['Plate Number'] || r['PlateNumber']) === (item['Plate Number'] || item['PlateNumber']) && r.Status === 'ENTERED');
    
    const entryTimeStr = item['Entry Time'] || item['Time'] || (oldRec ? (oldRec['Entry Time'] || oldRec['Time']) : '');
    const exitTimeStr = item['Exit Time'] || item['ExitTime'] || item['Time']; // sometimes exit is recorded in Time if it's the only one
    
    let entryTimeStrSafe = entryTimeStr ? entryTimeStr.replace(' ', 'T') : '';
    let exitTimeStrSafe = exitTimeStr ? exitTimeStr.replace(' ', 'T') : '';
    let entryTime = new Date(entryTimeStrSafe).getTime();
    let exitTime = exitTimeStrSafe ? new Date(exitTimeStrSafe).getTime() : Date.now();
    
    if (isNaN(entryTime)) entryTime = Date.now() - 60000;
    if (isNaN(exitTime)) exitTime = Date.now();
    
    const durationMs = exitTime - entryTime;
    const durationMinutes = Math.max(1, Math.floor(durationMs / 60000));
    const billedHours = Math.max(1, Math.ceil(durationMinutes / 60));
    const totalCost = billedHours * config.car;

    setExitQRData({
      plate: item['Plate Number'] || item['PlateNumber'],
      slot: item.Slot,
      entryTime: entryTimeStr,
      exitTime: exitTimeStr || new Date(exitTime).toLocaleString(),
      durationMinutes,
      totalCost
    });
  };
  ...
  return (
    <div className={`min-h-screen font-sans selection:bg-blue-500/30 p-4 md:p-8 transition-colors duration-500 ${isDark ? 'bg-[#020617] text-slate-200' : 'bg-slate-50 text-slate-800'}`}>
      ...
      <header className="max-w-7xl mx-auto flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
        <div>
          <div className="flex items-center gap-4 mb-1">
            ...
            <h1 className={`text-2xl font-black tracking-tight italic uppercase transition-colors ${isDark ? 'text-white' : 'text-slate-900'}`}>
      ...
      <main className="max-w-7xl mx-auto">
            ...
            <div className={`border p-6 rounded-[2rem] backdrop-blur-md h-full flex flex-col transition-colors ${isDark ? 'bg-slate-900/40 border-slate-800' : 'bg-white border-slate-200 shadow-[0_10px_40px_-15px_rgba(0,0,0,0.05)]'}`}>
              ...
              <div className="space-y-3 overflow-y-auto pr-2 custom-scrollbar flex-grow" style={{ maxHeight: '600px' }}>
                {data.slice().reverse().map((item, index) => (
                  <HistoryItem key={index} item={item} TOTAL_SLOTS={TOTAL_SLOTS} isDark={isDark} onShowQR={() => handleShowQR(item)} />
                ))}
              </div>
        ...
        {/* EXIT QR MODAL */}
        <ExitQRModal data={exitQRData} onClose={() => setExitQRData(null)} isDark={isDark} />
      </main>
    </div>
  );
};
...
const HistoryItem = ({ item, TOTAL_SLOTS, isDark, onShowQR }) => {
  ...
  return (
    <div 
      onClick={isExited ? onShowQR : undefined}
      className={`group border p-3 rounded-xl transition-all ${isExited ? 'cursor-pointer hover:scale-[1.02]' : ''} ${isDark ? 'bg-slate-800/20 border-slate-800/50 hover:bg-slate-800/40' : 'bg-slate-50 border-slate-100 hover:bg-slate-100'}`}
    >
      <div className="flex justify-between items-start mb-2">
        ...
        <div className="flex items-center gap-2">
          {isExited && (
            <span className={`flex items-center gap-1 text-[8px] uppercase tracking-widest font-bold px-1.5 py-0.5 rounded transition-all cursor-pointer hover:scale-105 shadow-sm ${isDark ? 'bg-blue-500/20 text-blue-400 hover:bg-blue-500/30' : 'bg-blue-100 text-blue-600 hover:bg-blue-200'}`}>
...
const ExitQRModal = ({ data, onClose, isDark }) => {
  if (!data) return null;
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
      <div className={`relative w-full max-w-sm p-6 rounded-3xl shadow-2xl border ${isDark ? 'bg-slate-900 border-slate-700' : 'bg-white border-slate-200'}`}>
        <button onClick={onClose} className={`absolute top-4 right-4 p-2 rounded-full transition-colors ${isDark ? 'bg-slate-800 text-slate-400 hover:text-white' : 'bg-slate-100 text-slate-500 hover:text-slate-800'}`}>
          <X size={20} />
        </button>
        ...
        <div className="text-center mb-6">
          ...
          <h2 className={`text-2xl font-black tracking-tight ${isDark ? 'text-white' : 'text-slate-800'}`}>Exit Detected</h2>
          <p className={`text-sm font-bold uppercase tracking-widest ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>Scan to Pay</p>
        </div>

        <div className="flex justify-center mb-6 p-4 bg-white rounded-2xl mx-auto w-fit shadow-sm">
          <QRCodeSVG value={qrPayload} size={200} level="H" includeMargin={true} />
        </div>

        <div className={`space-y-3 p-4 rounded-2xl text-sm ${isDark ? 'bg-slate-800/50 text-slate-300' : 'bg-slate-50 text-slate-600'}`}>
          <div className="flex justify-between items-center"><span className="font-bold uppercase tracking-widest text-[10px]">Plate</span> <span className="font-mono font-bold px-2 py-0.5 rounded border border-current">{data.plate}</span></div>
          <div className="flex justify-between items-center"><span className="font-bold uppercase tracking-widest text-[10px]">Slot</span> <span className="font-bold">{data.slot}</span></div>
          <div className="flex justify-between items-center"><span className="font-bold uppercase tracking-widest text-[10px]">Duration</span> <span className="font-bold">{data.durationMinutes} min</span></div>
          <div className="flex justify-between items-center pt-3 border-t border-slate-500/20"><span className="font-black uppercase tracking-widest text-lg">Total</span> <span className="font-black text-xl text-blue-500">₹{data.totalCost}</span></div>
        </div>
      </div>
    </div>
  );
