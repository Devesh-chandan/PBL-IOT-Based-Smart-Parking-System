...
const parkings = [
  ...
  },
  {
    ...
    image: 'https://images.unsplash.com/photo-1520106680373-c15a9a5f4c9b?auto=format&fit=crop&q=80&w=800'
  },
  {
    id: 'pavillion',
    ...
    <div className={`min-h-screen font-sans selection:bg-blue-500/30 overflow-x-hidden transition-colors duration-500 ${isDark ? 'bg-[#070b14] text-slate-200' : 'bg-slate-50 text-slate-800'}`}>
      ...
      <section className={`relative pt-24 pb-44 px-6 md:px-12 w-full flex flex-col justify-center overflow-hidden transition-colors duration-500 ${isDark ? 'bg-gradient-to-br from-[#0f172a] to-[#020617]' : 'bg-gradient-to-br from-slate-100 to-white'}`}>
        ...
        <div className="relative z-10 max-w-5xl mx-auto w-full flex flex-col items-center text-center pb-12">
          ...
          <h1 className={`text-6xl md:text-8xl lg:text-9xl font-black tracking-tighter mb-8 leading-[1.05] transition-colors min-h-[1.2em] flex flex-wrap justify-center items-center ${isDark ? 'text-white' : 'text-slate-900'}`}>
            <span>
              {typedText1}
          ...
          <p className={`text-lg md:text-2xl max-w-2xl font-medium mb-16 transition-colors ${isDark ? 'text-slate-400' : 'text-slate-600'}`}>
      ...
      <div className="relative z-20 max-w-5xl mx-auto px-6 md:px-12 -mt-10 md:-mt-12">
        <div className={`border rounded-2xl p-2.5 flex flex-col md:flex-row backdrop-blur-2xl transition-colors duration-500 ${isDark ? 'bg-[#0b1120]/95 border-blue-500/30 shadow-[0_0_40px_-10px_rgba(59,130,246,0.3)]' : 'bg-white/95 border-blue-200 shadow-blue-900/10'}`}>
          ...
          <div className={`flex-1 flex items-center gap-3 px-4 py-3 md:border-r transition-colors ${isDark ? 'border-slate-700' : 'border-slate-200'}`}>
          ...
          <div className={`flex items-center gap-3 px-4 py-3 md:border-r cursor-pointer transition-colors ${isDark ? 'border-slate-700 hover:bg-white/5' : 'border-slate-200 hover:bg-slate-50'}`}>
            <Calendar className="text-blue-500" size={20} />
            <input
              type="text"
              value={date}
              onChange={(e) => setDate(e.target.value)}
              className={`w-28 bg-transparent border-none focus:outline-none text-sm font-medium transition-colors ${isDark ? 'text-white' : 'text-slate-800'}`}
            />
          </div>

          <div className={`flex items-center gap-3 px-4 py-3 mr-2 cursor-pointer transition-colors ${isDark ? 'hover:bg-white/5' : 'hover:bg-slate-50'}`}>
            <Clock className="text-blue-500" size={20} />
            <input
              type="text"
              value={time}
              onChange={(e) => setTime(e.target.value)}
              className={`w-20 bg-transparent border-none focus:outline-none text-sm font-medium transition-colors ${isDark ? 'text-white' : 'text-slate-800'}`}
            />
          </div>
